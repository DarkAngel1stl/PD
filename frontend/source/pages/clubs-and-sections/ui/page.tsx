...
import { IClub, ClubsList } from '@/entities/club';
...
export const Page = () => {
  ...
  const fetchClubs = async () => {
    setLoading(true);
    let query =
      'https://social-programs-portal-backend.vercel.app/api/publications/clubs?';
    if (search) {
      query += `search=${search}&`;
    }
    if (selectedCities) {
  ...
  useEffect(() => {
    if (!mounted) {
      fetchClubs().finally(() => setMounted(true));
    }
  }, []);
  ...
  return (
    <div className="flex flex-col gap-y-4">
      ...
      <ClubsList clubs={clubs} mounted={mounted} />
    </div>
  );
};
