...
import { ClubFormType, clubFormSchema, photoSchema } from '../model/schemas';
import {
  Form,
  FormControl,
  FormDescription,
...
export const ClubCreateForm = () => {
  ...
  const onSubmit = async (data: ClubFormType) => {
    if (!data.terms) {
      toast(
    ...
    const res = await axios.post(
      'https://social-programs-portal-backend.vercel.app/api/publications/clubs/create',
      newData
    );
    setOpen(true);
  };
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-x-4 gap-y-3 lg:flex-row">
            <div className="w-full">
              <FormField
                control={form.control}
                name="ageCategory.from"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Возрастное ограничение (от) <span>*</span>
                    </FormLabel>
                    <FormControl>
                      <Input
                        min="0"
            ...
            <div className="w-full">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Возрастное ограничение (до) <span>*</span>
                    </FormLabel>
                    <FormControl>
                      <Input
                        min="0"
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Выберите направление <span>*</span>
                </FormLabel>
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  >
                    <SelectGroup>
                      <SelectItem value="section">Спортивные секции</SelectItem>
                      <SelectItem value="club">Кружки</SelectItem>
                    </SelectGroup>
                  </SelectContent>
                </Select>
                <FormMessage />
